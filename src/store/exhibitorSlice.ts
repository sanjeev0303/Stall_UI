...
export const fetchExhibitors = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      // Don't throw error for empty data - let fallback handle it
      if (response.data.length === 0) {
        console.log('⚠️ Redux: API returned empty array - will use fallback data')
    ...
    //   console.error('❌ Redux: API failed with error:', errorMessage)
      console.log('🔄 Redux: Switching to fallback mock data due to API failure')
      ...
      // Since we have fallback data, we don't want to reject the promise
      // Instead, we return the fallback data as a successful response
